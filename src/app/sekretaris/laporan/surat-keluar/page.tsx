...
import { FiSend, FiCalendar, FiDownload, FiPrinter } from "react-icons/fi";

export default function SekretarisLaporanSuratKeluarPage() {
  const [periode, setPeriode] = useState("januari-2025");
  
  const dataBulan = [
    { bulan: "Januari", total: 38, menungguTTD: 8, terkirim: 30 },
    { bulan: "Desember", total: 35, menungguTTD: 5, terkirim: 30 },
    { bulan: "November", total: 40, menungguTTD: 6, terkirim: 34 },
  ];

  const dataTujuan = [
    { tujuan: "Kecamatan", jumlah: 12, persentase: 32 },
    { tujuan: "Dinas", jumlah: 10, persentase: 26 },
    { tujuan: "BPD", jumlah: 8, persentase: 21 },
    { tujuan: "Lainnya", jumlah: 8, persentase: 21 },
  ];
    ...
    <section>
      ...
      <div className="mb-8">
        <h2 className="text-3xl font-bold text-gray-800 flex items-center gap-3">
          <FiSend className="text-blue-600" /> Laporan Surat Keluar
        </h2>
        <p className="text-gray-500 mt-1">
          Laporan dan statistik surat keluar berdasarkan periode
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
              ...
              <div key={i} className="border-b pb-3 last:border-0">
                ...
                <div className="w-full bg-gray-200 rounded-full h-2.5">
                  <div 
                    ...
                    style={{ width: `${(item.total / 45) * 100}%` }}
                  ></div>
                </div>
                <div className="flex justify-between text-xs text-gray-500 mt-1">
                  <span>Menunggu: {item.menungguTTD}</span>
                  <span>Terkirim: {item.terkirim}</span>
                </div>
              </div>
            ))}
          </div>
        ...
        {/* Tujuan Surat */}
        <div className="bg-white rounded-lg border border-gray-200 shadow-sm p-6">
          <h3 className="text-lg font-semibold text-gray-800 mb-4">Distribusi Tujuan Surat</h3>
          <div className="space-y-4">
            {dataTujuan.map((item, i) => (
              <div key={i} className="border-b pb-3 last:border-0">
                <div className="flex justify-between items-center mb-2">
                  <span className="font-medium text-gray-700">{item.tujuan}</span>
      ...
      <div className="bg-white rounded-lg border border-gray-200 shadow-sm p-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div className="text-center p-4 bg-orange-50 rounded-lg">
            ...
            <p className="text-sm text-gray-600">Menunggu TTD</p>
          </div>
