...
export default function PreviewSekretarisSuratKeluarPage() {
    ...
    const fetchDetail = async () => {
      ...
      try {
        ...
        if (!response.ok || !result.success) {
          throw new Error(result.error || "Gagal memuat data preview surat keluar");
        }

        setDetail(result.data as SuratKeluarDetail);
      } catch (err) {
        setError(err instanceof Error ? err.message : "Terjadi kesalahan saat memuat preview surat keluar");
      } finally {
        setLoading(false);
      }
    };
  ...
  const title = detail ? `Nomor Surat ${detail.nomor_surat}` : "Preview Surat Keluar";
    ...
    <section className="min-h-screen bg-slate-50 px-4 py-6 md:px-6">
      ...
      <div className="mx-auto max-w-6xl overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div className="flex flex-wrap items-center justify-between gap-4 border-b border-slate-200 px-6 py-5">
          <div>
            <h1 className="text-2xl font-bold text-slate-900">Preview Surat Keluar</h1>
            <p className="text-sm text-slate-500">{detail ? title : "Memuat dokumen surat keluar"}</p>
          </div>
