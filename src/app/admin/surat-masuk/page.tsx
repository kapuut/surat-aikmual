...
export default function SuratMasukPage() {
  ...
  const filteredSuratMasuk = suratMasuk.filter((item) => {
    ...
    const itemDate = parseValidDate(item.tanggal_surat);
    const itemDateValue = itemDate ? itemDate.toISOString().slice(0, 10) : "";
    ...
    const dayFromMatch = !dayMin || (itemDateValue !== "" && itemDateValue >= dayMin);
    const dayToMatch = !dayMax || (itemDateValue !== "" && itemDateValue <= dayMax);
  ...
  }, [filteredSuratMasuk.length, selectedDayFrom, selectedDayTo]);
    ...
    <section>
        ...
        {hasDateFilter && (
          <div className="mb-4 inline-flex items-center gap-2 rounded-lg border border-blue-200 bg-blue-50 px-4 py-2.5 text-sm text-blue-900">
            ...
            <span>{filterSummaryText.replace(`${filteredSuratMasuk.length} data`, "")}<strong>{filteredSuratMasuk.length}</strong> data</span>
          </div>
        )}
