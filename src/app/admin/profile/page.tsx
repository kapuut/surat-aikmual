...
export default function AdminProfilePage() {
  ...
  const fetchProfile = async () => {
    try {
      ...
      setProfileError(err instanceof Error ? err.message : "Gagal memuat profil admin");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleProfileSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setProfileError(err instanceof Error ? err.message : "Gagal menyimpan profil admin");
    } finally {
  ...
  if (loading) {
    return <div className="text-gray-500">Memuat profil akun admin...</div>;
  }
    ...
    <section className="space-y-6">
      <div className="mb-2">
        <h2 className="text-3xl font-bold text-gray-800 flex items-center gap-3">
          <FiUser className="text-blue-600" /> Profil Akun
        </h2>
        <p className="text-gray-500 mt-1">Kelola data akun dan ubah password administrator.</p>
      </div>
